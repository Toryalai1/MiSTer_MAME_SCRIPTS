...
download_mame_roms_from_mra() {
   ...
   do
         ...
         case "$VER" in
            ...
            'darksoft_neogeo')
                  curl ${CURL_RETRY} ${SSL_SECURITY_OPTION} --fail --location -o "${ZIP_PATH}" "https://archive.org/download/mister_neogeo/${f}"
                     ;;	  
            *)
                  echo "MAME version not listed in MRA or there is no download source for the version, downloading from .253 set"
	 ...
	 # Fallback
         if [ ! -s "$ROMMAME"/"${f}" ] ; then
             echo "MAME rom not found on $VER set, downloading from .253 set"
             curl ${CURL_RETRY} ${SSL_SECURITY_OPTION} --fail --location -o "${ZIP_PATH}" "https://archive.org/download/mame-merged/mame-merged/${f}"
	 fi
